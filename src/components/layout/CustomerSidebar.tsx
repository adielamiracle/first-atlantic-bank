...
export const CustomerSidebar: React.FC = () => {
  const { currentView, setCurrentView, currentUser, logout, switchToAdmin } = useBank();
  ...
  return (
    <aside className="w-64 bg-[#081728] border-r border-slate-800/90 text-slate-300 flex flex-col justify-between shrink-0 h-screen sticky top-0 overflow-y-auto select-none hidden lg:flex shadow-xl">
        ...
        <div className="mx-4 my-3.5 p-3 rounded-xl bg-[#0d233e] border border-[#c5a880]/20 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase font-bold tracking-wider text-[#c5a880]">
              {currentUser?.kycTier.replace(/_/g, ' ') || 'Private Client'}
            </div>
      ...
      {/* Bottom Administration / Sign Out */}
      <div className="p-4 border-t border-slate-800 space-y-2">
        <button
          onClick={() => switchToAdmin()}
          className="w-full flex items-center justify-between px-3 py-2 rounded-lg bg-amber-950/30 hover:bg-amber-950/60 text-amber-300 text-xs font-medium border border-amber-800/40 transition-colors"
        >
        ...
        <button
          onClick={() => logout()}
          className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-950/20 text-xs font-medium transition-colors"
        >
