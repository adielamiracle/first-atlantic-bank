...
import { TransferCodesScreen } from './TransferCodesScreen';
...
export const TransferRoutes: React.FC = () => {
  ...
  const getStepProgress = (pathname: string) => {
    ...
    if (pathname.includes('/beneficiary')) return 2;
    if (pathname.includes('/account')) return 3;
    if (pathname.includes('/codes')) return 4;
  ...
  return (
    <div className="min-h-full flex items-center justify-center py-3 sm:py-8 px-2 sm:px-4">
      <div className="w-full max-w-lg bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden transition-all">
        ...
        <Routes>
          ...
          <Route path="/transfer/codes" element={<TransferCodesScreen />} />
          ...
          <Route path="/codes" element={<TransferCodesScreen />} />
